...
*   **LLM (Büyük Dil Modeli):** Llama 3.2 (1B Parametre) - *Ollama üzerinden yerel çalıştırıldı.*
...
1.  **Tamamen Yerel ve Güvenli (Air-gapped):** Veriler dış sunuculara gitmez.
2.  **Dinamik PDF Analizi:** Kullanıcılar kendi siber güvenlik yönergelerini (PDF) arayüzden yükleyip anında sorgulayabilir.
3.  **Anti-Halüsinasyon (Strict Prompting):** Model, yalnızca sağlanan bağlam (context) üzerinden cevap verecek şekilde kısıtlanmıştır. Bağlam dışı sorularda uydurma yapmaz.
4.  **Hafızalı Sohbet (Chat History):** Önceki soruları ekranda tutarak kesintisiz bir deneyim sunar.
